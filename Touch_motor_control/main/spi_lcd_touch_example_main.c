...
// Public API functions that UI will call
void esc_init(void)
{
...
void esc_start_motor(void)
{
    if (target_rpm == 0) {
        ESP_LOGW(TAG, "Cannot start: target RPM is 0");
        return;
    }
    ESP_LOGI(TAG, "Starting motor to %lu RPM", target_rpm);
    start_rpm_transition(target_rpm);
}

void esc_stop_motor(void)
{
...
void esc_increase_target(uint32_t step)
{
    target_rpm += step;
    if (target_rpm > MAX_RPM) {
        target_rpm = MAX_RPM;
    }
    ESP_LOGI(TAG, "Target RPM increased to: %lu", target_rpm);
}

void esc_decrease_target(uint32_t step)
{
    if (target_rpm >= step) {
        target_rpm -= step;
    } else {
    ...
    ESP_LOGI(TAG, "Target RPM decreased to: %lu", target_rpm);
}

void esc_set_target(uint32_t rpm)
{
    if (rpm > MAX_RPM) rpm = MAX_RPM;
    target_rpm = rpm;
    ESP_LOGI(TAG, "Target RPM set to: %lu", target_rpm);
}

uint32_t esc_get_current_rpm(void)
{
    return current_rpm;
}

uint32_t esc_get_target_rpm(void)
{
    return target_rpm;
}

bool esc_is_transitioning(void)
{
    return is_transitioning;
}

uint32_t esc_get_max_rpm(void)
{
    return MAX_RPM;
}
...
static bool example_lvgl_touch_cb(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
    ...
    if (touchpad_pressed && touchpad_cnt > 0) {
        ...
        data->point.y = EXAMPLE_LCD_V_RES -  touchpad_y[0] ;
